...
$siswa = $koneksi->query("SELECT * FROM rb_siswa a 
                          JOIN rb_jenis_kelamin b ON a.id_jenis_kelamin=b.id_jenis_kelamin 
                          WHERE a.kode_kelas='$id_kelas' 
                          ORDER BY a.id_siswa");

while ($r = $siswa->fetch_assoc()) {
    // Hitung jumlah kehadiran
    $total = $koneksi->query("SELECT DISTINCT tanggal FROM rb_absensi_siswa WHERE kodejdwl='$kode_jadwal'")->num_rows;
    $hadir = $koneksi->query("SELECT * FROM rb_absensi_siswa WHERE kodejdwl='$kode_jadwal' AND nisn='{$r['nisn']}' AND kode_kehadiran='H'")->num_rows;
    $sakit = $koneksi->query("SELECT * FROM rb_absensi_siswa WHERE kodejdwl='$kode_jadwal' AND nisn='{$r['nisn']}' AND kode_kehadiran='S'")->num_rows;
    $izin = $koneksi->query("SELECT * FROM rb_absensi_siswa WHERE kodejdwl='$kode_jadwal' AND nisn='{$r['nisn']}' AND kode_kehadiran='I'")->num_rows;
    $alpa = $koneksi->query("SELECT * FROM rb_absensi_siswa WHERE kodejdwl='$kode_jadwal' AND nisn='{$r['nisn']}' AND kode_kehadiran='A'")->num_rows;
