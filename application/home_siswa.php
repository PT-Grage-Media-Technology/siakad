...
<?php
  ...
  $tampil = mysql_query("SELECT * FROM rb_journal_list z JOIN rb_guru t ON z.users=t.nip JOIN rb_forum_topic ft ON z.materi=ft.judul_topic WHERE z.kodejdwl, ft.kodejdwl='$_GET[kodejdwl]'");
  $no = 1;
  while ($r = mysql_fetch_array($tampil)) {
    // var_dump($r);
    echo "<tr><td>$no</td>
                            ...
                            <td>$r[materi]</td>
                            <td>$r[keterangan]</td>
                            <td><a class='btn btn-success btn-xs' title='Lihat Data' href='index.php?view=home&act=detailpembelajaran&kodejdwl=$r[kodejdwl]&tanggal=$r[tanggal]&jam_ke=$r[jam_ke]&id_topic=$r[id_forum_topic]'>
                        </tr>";
    $no++;
  }
  echo "<tbody>
