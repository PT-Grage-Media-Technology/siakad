...
if (isset($_POST['simpan'])) {
    $juml = mysqli_num_rows(mysqli_query($connection, "SELECT * FROM rb_siswa WHERE kode_kelas='$_GET[id]'"));
    for ($ia = 1; $ia <= $juml; $ia++) {
        $a = $_POST['a' . $ia];
        $b = $_POST['b' . $ia];
        $c = $_POST['c' . $ia];
        ...
        if (!empty($a) || !empty($b) || !empty($c)) {
            $cek = mysqli_num_rows(mysqli_query($connection, "SELECT * FROM rb_nilai_sikap WHERE kodejdwl='$_POST[jdwl]' AND nisn='$nisn' AND status='$_POST[status]'"));
            if ($cek >= 1) {
                mysqli_query($connection, "UPDATE rb_nilai_sikap SET positif='$a', negatif='$b', deskripsi='$c' WHERE kodejdwl='$_GET[jdwl]' AND nisn='$nisn' AND status='$_POST[status]'");
            } else {
                mysqli_query($connection, "INSERT INTO rb_nilai_sikap VALUES('', '$_GET[jdwl]', '$nisn', '$a', '$b', '$c', '$_POST[status]', '$_SESSION[id]', '" . date('Y-m-d H:i:s') . "')");
            }
        }
    }
...
$d = mysqli_fetch_array(mysqli_query($connection, "SELECT * FROM rb_kelas WHERE kode_kelas='$_GET[id]'"));
$m = mysqli_fetch_array(mysqli_query($connection, "SELECT * FROM rb_mata_pelajaran WHERE kode_pelajaran='$_GET[kd]'"));
...
echo "<div class='col-md-12'>
          ...
          <div class='panel-body'>
              <ul id='myTabs' class='nav nav-tabs' role='tablist'>
                  ...
                  <li role='presentation'><a href='#sosial' role='tab' id='sosial-tab' data-toggle='tab' aria-controls='sosial' aria-expanded='false'>Penilaian Sosial</a></li>
                  <li role='presentation'><a href='#pengetahuan' role='tab' id='pengetahuan-tab' data-toggle='tab' aria-controls='pengetahuan' aria-expanded='false'>Penilaian Pengetahuan</a></li>
              </ul><br>
...
echo "<div class='col-md-12'>
      ...
      <tbody>";
$no = 1;
$tampil = mysqli_query($connection, "SELECT * FROM rb_siswa WHERE kode_kelas='$_GET[id]' ORDER BY id_siswa");
while ($r = mysqli_fetch_array($tampil)) {
    $des = mysqli_fetch_array(mysqli_query($connection, "SELECT * FROM rb_nilai_sikap WHERE kodejdwl='$_GET[jdwl]' AND nisn='$r[nisn]' AND status='spiritual'"));
    echo "<tr>
              <td>$no</td>
              <td>$r[nisn]</td>
              <td>$r[nama]</td>
